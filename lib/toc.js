...
// Pollution guard: NODE_ENV=test 時は env 必須（test/_env.js 参照）
if (process.env.NODE_ENV === 'test' && !process.env.LAWS_JP_TOC_DIR) {
  throw new Error(
    'toc: refusing to fall back to ~/.cache/laws-jp/toc in test mode. ' +
    'Set LAWS_JP_TOC_DIR=<isolated-path> BEFORE require() to avoid polluting the production cache. ' +
