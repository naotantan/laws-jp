...
function buildSystemPrompt(query, refsForPrompt, warnings = '') {
  ...
  const pattern = QUERY_PATTERNS.find(p => p.id === patternId) || QUERY_PATTERNS.find(p => p.id === 'comprehensive');
  const sections = PATTERN_SECTIONS[patternId].map(s => s).join('\n');

  // refsForPrompt and warnings are appended in sections below; no placeholders in template.
