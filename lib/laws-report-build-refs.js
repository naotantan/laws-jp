...
async function fetchLawSafe(lawName) {
  try {
    ...
    const totalBytes = articles.reduce((s, a) => s + (a.byte_size ?? Buffer.byteLength(a.body_md || '', 'utf8')), 0);
    return { meta: flatMeta, articles, totalBytes };
  } catch (e) {
    process.stderr.write(`[laws-report] WARN fetchLawSafe "${lawName}": ${e.message}\n`);
    return null;
  }
}
...
function detectMentionedArticles(query, allRefs) {
  ...
  for (const m of query.matchAll(pattern)) {
    ...
    if (!p) continue;
    const match = allRefs.find(r =>
      r.article_num_int === p.num &&
      r.article_sub === p.sub &&
      r.article_sub_sub === (p.subSub || 0)
    );
    if (match) {
...
async function buildRefs(opts) {
  ...
  // estMap preserves lawName→estimatedName before reordering so divergence check stays aligned.
  const estMap = new Map(lawNames.map((n, i) => [n, estimatedNames[i] || n]));
  const paired = (await Promise.all(fetchTargets.map(async n => {
    const r = await fetchLawSafe(n);
    return r ? { result: r, estimatedName: estMap.get(n) || n } : null;
  }))).filter(Boolean);
  ...
  if (totalBytes > TOC_FIRST_THRESHOLD && articleIds.length === 0) {
    const toc = results.map(r => ({
      ...
      articles: r.articles.map(a => ({
        ...
        caption: a.caption || '',
        byte_size: a.byte_size ?? Buffer.byteLength(a.body_md || '', 'utf8'),
      })),
    }));
    return {
      mode: 'toc-first',
  ...
  const refs = allArticles.slice(0, MAX_ARTICLES).map((art, i) => ({
    ...
    article_sub_sub: art.article_sub_sub || 0,
    caption: art.caption || '',
  ...
  // Divergence: estimated name vs actual e-Gov title (uses paired to avoid index misalignment after filter)
  for (const { result, estimatedName } of paired) {
    const sim = bigramSimilarity(estimatedName, result.meta.law_title);
    if (sim < DIVERGENCE_THRESHOLD) {
      warnings.divergence = (warnings.divergence ? warnings.divergence + '\n' : '') +
        `⚠️ 推定法令名「${estimatedName}」とe-Gov取得名「${result.meta.law_title}」の類似度: ${sim.toFixed(2)}（しきい値${DIVERGENCE_THRESHOLD}未満）。法令名が乖離しています。`;
    }
  }
